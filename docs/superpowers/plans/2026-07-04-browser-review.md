...
**Goal:** Build a zero-dependency browser UI (`make review-browser`) for browsing and rating dataset rows as Good/Unclear/Bad, plus a Generate tab that creates live rows from a typed message.

**Architecture:** A single Python file (`scripts/review_server.py`) uses `http.server.BaseHTTPRequestHandler` (stdlib only). Helper functions handle feedback I/O and row sampling. The full HTML/CSS/JS UI is embedded as a Python string constant and served at `GET /`. The server runs from the project root so all existing relative paths (`../datasets/…`) work unchanged.

**Tech Stack:** Python 3.9 stdlib only (`http.server`, `json`, `urllib.parse`, `webbrowser`). Existing project scripts: `config.py`, `scenarios.py`, `templates.py`, `responses.py`.

## Global Constraints

- Zero new dependencies — stdlib only, no Flask, no aiohttp.
- Server MUST `os.chdir` to the project root on startup so `../datasets/…` paths resolve correctly.
- All feedback writes append to JSONL files (never overwrite).
...
- Browser-generated row IDs: `browser_generated_<unix_ms_timestamp>`.
- Default port: 8765. CLI flags: `--port N`, `--open`.
- `make review-browser` target added after the existing `quality` target in `Makefile`.
- Guide saved to `docs/browser-review-guide.md`.
- Tests follow repo pattern: `sys.path.insert(0, …/scripts)`.
- All tests run with `python3 -m pytest tests/ -v` from the project root.

---

### Task 1: Core server helpers + tests

**Files:**
- Create: `scripts/review_server.py` (helper functions only — no HTTP handler yet)
- Create: `tests/test_review_server.py`

**Interfaces:**
- Produces:
  - `load_rows(path: Path) -> list[dict]`
  - `load_reviewed_ids(feedback_dir: Path) -> set[str]`
  - `sample_row(rows: list[dict], reviewed_ids: set[str], mode: str = "random") -> dict | None`
  - `build_generated_row(domain: str, task_type: str, intent: str, message: str) -> dict`
  - `rate_row(row_id: str, rating: str, comment: str, row: dict, feedback_dir: Path) -> None`
...
python3 -m pytest tests/test_review_server.py -v
```

Expected: `ModuleNotFoundError: No module named 'review_server'` (or similar — file doesn't exist yet).

- [ ] **Step 3: Create `scripts/review_server.py` with helpers only (no HTTP handler yet)**
...
def sample_row(rows: list, reviewed_ids: set, mode: str = "random") -> dict | None:
...
```bash
git add scripts/review_server.py tests/test_review_server.py
git commit -m "feat: add review_server.py helper functions with tests"
```

---

### Task 2: HTTP handler + embedded HTML/CSS/JS UI

**Files:**
- Modify: `scripts/review_server.py` — replace `_HTML` placeholder and `ReviewHandler` stub with complete implementations

**Interfaces:**
- Consumes (from Task 1): `load_rows`, `load_reviewed_ids`, `sample_row`, `build_generated_row`, `rate_row`, `PROCESSED_PATH`, `FEEDBACK_DIR`, `DOMAINS`, `TASK_TYPES`, `CUSTOMER_INTENTS`
- Produces: working server at `http://localhost:8765` with all 7 endpoints

- [ ] **Step 1: Replace `_HTML` constant with the full UI**

Replace the `_HTML = "..."` line and everything after it up to (but NOT including) `class ReviewHandler`) with:
...
- [ ] **Step 2: Replace `ReviewHandler` stub with the full HTTP handler**

Replace the entire `class ReviewHandler` block with:
...
curl -s http://localhost:8765/ | grep -o "LusoSupport"
# Expected: LusoSupport

# Options endpoint
curl -s http://localhost:8765/api/options | python3 -m json.tool | head -10
# Expected: {"domains": [...], "task_types": [...], "intents": [...]}

# Sample endpoint
curl -s "http://localhost:8765/api/sample?mode=random" | python3 -m json.tool | grep '"id"'
# Expected: "id": "lusosupport_pt_..."

# Rate endpoint
curl -s -X POST http://localhost:8765/api/rate \
  -H "Content-Type: application/json" \
  -d '{"id":"smoke_test_id","rating":"unclear","comment":"smoke","row":{}}' \
  | python3 -m json.tool
# Expected: {"ok": true}
```
...
Expected: all 57 tests pass (40 existing + 17 new).

- [ ] **Step 5: Commit**

```bash
git add scripts/review_server.py
git commit -m "feat: add browser review UI — full HTTP handler and embedded HTML/CSS/JS"
```

---

### Task 3: Makefile target + user guide

**Files:**
- Modify: `Makefile` — add `review-browser` target after `quality`
- Create: `docs/browser-review-guide.md`

**Interfaces:**
- Consumes: `scripts/review_server.py --port 8765 --open` (from Task 2)

- [ ] **Step 1: Add `review-browser` to `.PHONY` and add the target**

In `Makefile`, find the `.PHONY` line and add `review-browser` to it. Then add the target after `quality`:
...
Note: the indentation MUST be a tab character, not spaces.

- [ ] **Step 2: Verify `make review-browser` starts the server**
...
curl -s http://localhost:8765/ | grep -o "LusoSupport"
# Expected: LusoSupport
kill %1  # stop background make
```

- [ ] **Step 3: Create `docs/browser-review-guide.md`**
...
```bash
git add Makefile docs/browser-review-guide.md
git commit -m "feat: add make review-browser target and browser review guide"
```
